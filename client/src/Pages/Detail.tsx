...
export const DetailPage = (props: { toggle: string }) => {
  ...
  useEffect(() => {
    if (props.toggle === "class") {
      axios.post("/class/search", {
        ...
        .then((res) => setDisplay(res.data.class as IClass))
        .catch((err) => alert("Class not found!"));
    } else if (props.toggle === "product") {
      axios.post("/product/search", {id:id})
      .then(res => setDisplay(res.data.product as IProduct))
  ...
  // };
  // console.log(formatDate(display?.registrationDate));
            ...
            <div className="content">
              {/* <table>
                ...
                <tbody>
                  ...
                  {display?.classDate?.map((ele :IClassDate) => (
                    <>
                      <td>{ele.startDate}</td>
                      <td>{ele.endDate}</td>
                    </>
